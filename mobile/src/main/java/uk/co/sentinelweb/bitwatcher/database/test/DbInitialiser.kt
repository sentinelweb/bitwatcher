...
class DbInitialiser @Inject constructor(val db: BitwatcherDatabase) {
    fun init(): Single<Boolean> {
        return Single.fromCallable({
            ...
            db.positionItemDao().insertPositionItem(PositionItemEntity(null, BTC, BigDecimal("3.6789"), 1))
            ...
            db.tickerDao().insertTicker(TickerEntity(8, "basic", BCH, EUR, BigDecimal.ZERO, Date()))
            true
        })
